...
protected:
	void init(int screenW, int screenH) {
		...
		switch (GetRandomValue(0, 3)) {
		case 0:
			texture = LoadTexture("trzask.png");
			scale = .25f * (float)render.size;
			break;
		case 1:
			texture = LoadTexture("senyszyn.png");
			scale = .4f * (float)render.size;
			break;
		case 2:
			texture = LoadTexture("Maciak.png");
			scale = .1f * (float)render.size;
			break;
		default:
			texture = LoadTexture("barto.png");
			scale = .1f * (float)render.size;
			break;
		}
		...
		ang = Utils::RandomFloat(0, 2 * PI);
		rad = Utils::RandomFloat(0, maxOff);
		Vector2 center = {
...
public:
	...
	void Draw() const override {
		DrawRectangleLines(transform.position.x - (texture.width * scale) * 0.5f, transform.position.y - (texture.height * scale) * 0.5f, texture.width*scale, texture.height*scale, WHITE);
		Vector2 dstPos = {
...
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			Rectangle l2 = { transform.position.x - 20.f, transform.position.y - LASER_LENGTH, 4.f, LASER_LENGTH };
			DrawRectangleRec(lr, RED);
			DrawRectangleRec(l2, RED);
		}
	}
	Vector2 GetPosition() const {
		return transform.position;
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("krzysztof-stanowski.png");
		...
		while (!WindowShouldClose()) {
			...
			// Asteroid-Ship collisions
			{
				auto remove_collision =
					...
					if (player->IsAlive()) {
						Vector2 transform = asteroid_ptr_like->GetPosition();
						Texture2D texture = asteroid_ptr_like->GetTexture();
						float scale = asteroid_ptr_like->GetScale();
						Rectangle rec = { transform.x - (texture.width * scale) * 0.5f, transform.y - (texture.height * scale) * 0.5f, texture.width * scale, texture.height * scale};

						if (CheckCollisionCircleRec(player->GetPosition(), player->GetRadius(), rec)) {
